...
if [ "$MODE" != "fast" ]; then
    ...
    # Той же командой, что и CI. Своя выдумка здесь уже подвела: --maxWorkers=50%
    # казался разумным против нехватки памяти, а на деле уронил тесты со
    # свойствами, которые чувствительны к числу рабочих процессов. Совпадение с
    # CI важнее догадок о том, как лучше.
    step "Тесты фронтенда"                  npm run test:coverage:ci --workspace=apps/web
fi
