...
Скрипт только читает, и обеспечивает это Postgres: сессия открывается с
`default_transaction_read_only=on`, и запись отклоняет сервер. Формулировка
запроса тут ничего не меняет — проверено на `SELECT … INTO`, который список
запрещённых слов пропускает, а база не выполняет.
