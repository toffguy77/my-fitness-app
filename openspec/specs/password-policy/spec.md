...
## Purpose
TBD - created by archiving change enforce-password-policy. Update Purpose after archive.
## Requirements
...
### Requirement: Эндпоинт смены пароля зарегистрирован и достижим

Обработчик смены пароля SHALL быть зарегистрирован в роутере по пути `/api/v1/auth/change-password` и достижим через тот же путь, который вызывает клиентское приложение. Наличие реализованного обработчика без его регистрации в роутере MUST считаться незавершённой работой.

#### Scenario: Маршрут отвечает не 404

- **WHEN** выполняется `POST /api/v1/auth/change-password` с валидным токеном аутентификации
- **THEN** ответ имеет статус, отличный от `404`

#### Scenario: Путь клиента совпадает с зарегистрированным маршрутом

- **WHEN** сравнивается путь, который вызывает клиентское приложение при смене пароля, с зарегистрированными маршрутами API
- **THEN** такой маршрут существует

#### Scenario: Смена пароля через интерфейс

- **WHEN** аутентифицированный пользователь меняет пароль на странице настроек, указав корректный текущий пароль и новый пароль, удовлетворяющий правилам
- **THEN** пароль изменён
- **AND** повторный вход с новым паролем выполняется успешно
- **AND** вход со старым паролем отклоняется
