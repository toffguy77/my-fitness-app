...
- [x] 11.1 Добавить сквозной тест Playwright: вход → настройки → смена пароля → успех → вход с новым паролем. Проверка: `e2e/tests/change-password.spec.ts`, на отдельном засеянном аккаунте — смена пароля завершает все сессии пользователя, и общий аккаунт разлогинил бы остальную часть прогона.
...
- [x] 12.1 Задеплоить на dev, вручную сменить пароль через `/settings/password`. Проверка: пароль меняется, старый не работает, сессии на других устройствах завершены. **Закрыто прогоном E2E 2026-09-12** (`change-password.spec.ts`, все сценарии зелёные): форма объясняет правила и отклоняет слабый пароль; несовпадение подтверждения ловится до запроса; неверный текущий пароль сообщается именно как неверный текущий, а не как общая ошибка; новый пароль работает, старый — нет. Завершение чужих сессий проверяется отдельно интеграционным `session_integration_test.go` — там же, где ловился оживавший refresh-токен.
- [x] 12.2 Повторить проверку на prod. Проверка: чек-лист пройден. **Политика проверена живьём на проде 2026-09-14** — на публичном маршруте регистрации, где отказ ничего не создаёт: короткий пароль, пароль без цифр, частый пароль и пароль из одних цифр отклонены, каждый с указанием причины. **Сама смена пароля через `/settings/password` на проде не проверялась:** для неё нужна учётная запись в боевой среде, а заводить её с известным паролем нельзя. Поведение смены закрыто прогоном E2E (задача 12.1).

  **Проверка нашла то, ради чего её и делают живьём.** Человек на проде видел:

  ```
  пароль не соответствует требованиям: [Пароль должен содержать хотя бы одну цифру]:
  password does not meet policy
  ```

  — английский хвост внутренней ошибки и квадратные скобки из вывода Go в тексте, который читает человек, пытающийся придумать пароль. Причина: обработчик отдавал `err.Error()` целиком.

  **И хуже, рядом:** регистрация отдавала `err.Error()` на **любую** ошибку. При занятом адресе человек получал `duplicate key value violates unique constraint "users_email_key"` — вместе с устройством нашей базы. Теперь занятый адрес распознаётся по коду `23505` и отвечает `409` с человеческим текстом, политика — `422` с причинами, всё остальное — общим сообщением и записью в журнал. Проверено сломом.
