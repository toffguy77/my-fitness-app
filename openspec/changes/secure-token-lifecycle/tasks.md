...
- [ ] 9.3 Убрать `refresh_token` из тел ответов и приём refresh из тела запроса. Проверка: тест «тело ответа входа не содержит `refresh_token`», e2e зелёные. **Осознанно не сделано раньше времени.** Приём из тела существует ради вкладок, открытых до перехода на cookie: их сборка умеет только так. Убрать его, пока доля не дошла до нуля, — значит разлогинить тех, кто просто давно не перезагружал страницу. Инструмент для решения появился в 9.2; делать по его показаниям, а не по ощущению, что пора.
