...
- [ ] 9.2 Задеплоить на prod, наблюдать долю `/auth/refresh` с телом запроса. Проверка: доля упала до нуля в течение суток. **Прод выкачен 2026-09-11, но наблюдать было нечем — теперь есть.** Источник токена нигде не записывался: `fromCookie` решал только судьбу cookie, а ответ на вопрос «можно ли уже убирать приём из тела» пришлось бы искать в логах по `body_size`, что ненадёжно — пустое тело неотличимо от отсутствующего. Добавлены два события, `refresh_from_cookie` и `refresh_from_body`, и панель на дашборде. Запрос: `sum(rate(burcev_domain_events_total{event="refresh_from_body"}[1h]))`. Ждём сутки живого трафика после выкатки этой правки.
