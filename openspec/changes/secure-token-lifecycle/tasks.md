...
- [x] 4.1 Реализовать `POST /api/v1/auth/ws-ticket`: 32 байта из `crypto/rand`, хеш в `ws_tickets`, TTL 30 секунд, удаление просроченных строк при каждой выдаче. Проверка: unit-тест на непредсказуемость и срок жизни.
- [x] 4.2 Переписать `chat/handler.go:353-372` на обмен тикета: атомарно пометить `used_at` и получить `user_id`; отклонять использованный, просроченный и любой запрос с параметром `token`. Проверка: тесты четырёх сценариев спека.
- [x] 4.3 Обновить `useWebSocket.ts:42-53` и `WebSocketProvider.tsx:39-50`: перед подключением запросить тикет, подключаться с `?ticket=`. Проверка: `npx jest useWebSocket WebSocketProvider` зелёные, URL содержит `ticket=`.
- [x] 4.4 Добавить ограничение частоты на `ws-ticket` и `resend-verification` в `auth_rate_limiter.go`. Проверка: тест «превышение лимита → 429, письмо не отправлено».

## 5. Маскирование логов

- [x] 5.1 В `middleware/logger.go` добавить маскирование значений query-параметров `token`, `ticket`, `refresh_token`, `code`, `state`, `password`. Проверка: тесты двух сценариев спека.
