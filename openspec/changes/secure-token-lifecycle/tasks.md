...
- [x] 4.3 Обновить `WebSocketProvider.tsx`: перед подключением запросить тикет, подключаться с `?ticket=`. Проверка: `npx jest useWebSocket WebSocketProvider` зелёные, URL содержит `ticket=`.
