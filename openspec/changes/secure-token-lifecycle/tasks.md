...
- [ ] 2.1 Реализовать `revokeAllUserRefreshTokens` как экспортируемый метод и вызвать его из `reset_service.go:ResetPassword` и из `service.go:ChangePassword`. Проверка: тест «после сброса все токены пользователя имеют `revoked_at`».
- [ ] 2.2 Заменить заглушку `InvalidateUserSessions` (`reset_service.go:378-392`) реальной реализацией: отзыв refresh-токенов + инкремент `users.token_version` в одной транзакции. Проверка: тест сценария «Проверка реализации отзыва».
