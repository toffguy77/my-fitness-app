...
- [ ] 9.3 Убрать `refresh_token` из тел ответов и приём refresh из тела запроса. **Осознанно не сделано: решать по показаниям, а не по ощущению, что пора.** Приём из тела существует ради вкладок, открытых до перехода на cookie. Убрать его, пока доля не дошла до нуля, — значит разлогинить тех, кто просто давно не перезагружал страницу. Инструмент для решения есть (задача 9.2), а показаний пока нет: за двое суток на `/auth/refresh` пришёл один запрос, и тот от сканера.
