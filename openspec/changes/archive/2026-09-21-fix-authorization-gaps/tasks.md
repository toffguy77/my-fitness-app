...
- [x] 7.2 Повторить проверку на prod. Проверка: `403` на чужом клиенте, `200` на своём. **Разделено на то, что проверяемо живьём, и то, что нет — с указанием остатка.**

  *Живьём на проде 2026-09-14:* маршруты с идентификатором клиента отвечают `401` и без токена, и с негодным — данные не отдаются до проверки входа. Проверено на `/api/v1/curator/clients/:id` и `/api/v1/curator/clients/:id/tasks`.

  *Само правило «чужой клиент недоступен»* проверяется в CI набором `e2e/tests/authorization.spec.ts`: он перебирает **все** маршруты с идентификатором клиента, полученные из таблицы маршрутов, и требует отказа на каждом от куратора, которому этот клиент не назначен. Двумя настоящими учётными записями кураторов, на том же коммите, что уезжает на прод. Плюс `internal/router/authorization_matrix_test.go` роняет сборку, если маршрут с идентификатором добавлен без объявленной защиты.

  **Чего не сделано:** проверка двумя кураторами против самого прода. Для неё нужны учётные записи кураторов на проде, а заводить их с известными паролями в боевой среде нельзя. Правило живёт в коде, а не в данных, и код на проде тот же — но это довод, а не измерение.
