...
- [x] 7.1 Задеплоить на dev, вручную проверить сценарий чужого клиента через два кураторских аккаунта. Проверка: второй куратор получает `403`. *Автоматизировано вместо ручной проверки: в затравку добавлен второй куратор без клиентов, `e2e/tests/authorization.spec.ts` обходит каждый маршрут с идентификатором клиента и требует отказа, плюс проверяет, что чужой клиент не виден и в списке.*
