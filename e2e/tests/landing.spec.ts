...
test.describe('Посадочная страница', () => {
    ...
    // Вход и регистрация обязаны отличаться не подписью, а существом: до
    // этого оба перехода открывали одну и ту же форму ссылки с блоком
    // согласий, и владелец продукта, проверяя руками, сообщил, что «вход» и
    // «регистрация» ведут на одну и ту же страницу.
    test('«Войти» в шапке открывает вход паролем, без согласий', async ({ page }) => {
        const header = page.getByRole('banner')
        await header.getByRole('link', { name: 'Войти' }).click()

        await expect(page).toHaveURL(/\/auth$/, { timeout: 10000 })

        await expect(page.getByLabel('Электронная почта')).toBeVisible({ timeout: 10000 })
        await expect(page.getByLabel('Пароль', { exact: true })).toBeVisible()

        // Согласия — признак заведения аккаунта, и на входе их видно быть не
        // должно.
        //
        // Здесь именно видимость, а не toHaveCount(0): форма ссылки остаётся
        // смонтированной под `hidden` (иначе переключение туда-обратно
        // стирало бы введённое), поэтому её согласия в разметке есть всегда.
        // Ноль в подсчёте означал бы, что форму снова размонтируют — то есть
        // требовал бы вернуть прежний дефект.
        await expect(page.getByText(/Я принимаю/).first()).toBeHidden()

        // Вход по ссылке остаётся в одном нажатии — для тех, у кого пароля нет.
        await expect(page.getByRole('button', { name: 'Войти по ссылке' })).toBeVisible()
    })

    // Регистрация — ссылка на почту с согласиями. Её отличие от входа теперь
    // видно без чтения заголовка: другая форма, другой набор полей.
