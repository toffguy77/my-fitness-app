...
function NutritionPageContent() {
  ...
  // Функция отправки куратору (завершение дня)
  const handleSubmit = async () => {
    ...
    try {
      ...
      logger.info('Nutrition: начало отправки отчета куратору', {
        userId: user.id,
        date: selectedDate,
        dayType,
        totals: {
      ...
      logger.userAction('Nutrition: отправка отчета куратору', {
        userId: user.id,
        date: selectedDate,
        dayType,
        mealsCount: dateMeals.length,
      ...
      if (error) {
        ...
        logger.info('Nutrition: отчет успешно отправлен куратору', {
          userId: user.id,
          date: selectedDate,
          dayType,
        })
        logger.userAction('Nutrition: отчет успешно отправлен куратору', {
          userId: user.id,
          date: selectedDate,
          dayType,
          mealsCount: dateMeals.length,
        ...
        setIsCompleted(true)
        setStatus('submitted')
        toast.success('Отчет отправлен куратору')
        ...
        // Остаемся на странице ввода; отправка куратору доступна с дашборда
        setTimeout(() => {
          setStatus('idle')
        }, 1200)
      }
  ...
  return (
    <main className="w-full min-h-screen bg-zinc-950 p-4 sm:p-6 lg:max-w-4xl lg:mx-auto font-sans">
        ...
        <div className="mb-6">
          ...
          {!currentTargets && (
            <div className="mt-3 p-3 bg-amber-950/20 border border-amber-800/50 rounded-lg text-sm text-amber-300">
              Цели для этого типа дня не установлены. Обратитесь к куратору.
            </div>
          )}
        </div>
      )}
      ...
      <div className="bg-zinc-900 p-6 rounded-2xl shadow-sm border border-zinc-800 space-y-6">
        ...
        {/* Кнопка сохранения черновика. Отправка куратору доступна только с дашборда */}
