...
export default function ReportsPage() {
  ...
  // Показываем paywall для бесплатных пользователей
  if (!isPremium) {
    return (
          ...
          {today.weight && (
            <div className="pt-2 border-t border-gray-100">
              <p className="text-xs text-gray-600">
                <span className="font-semibold">Вес:</span> {today.weight} кг
              </p>
            </div>
          )}
          {today.notes && (
            <p className="text-xs text-gray-600 whitespace-pre-line mt-2">Комментарий: {today.notes}</p>
          )}
        </section>
      ) : (
