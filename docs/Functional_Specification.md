...
- Опционально: инвайт-код координатора (`?code=XXXXX`)
- Автоматическое создание профиля с `role = 'client'`
- Дефолтные значения: `subscription_status = 'free'`, `subscription_tier = 'basic'`
- Автоматическое назначение координатора при использовании инвайт-кода
...
#### 3.3 Виджет заметки координатора (Premium)
- Отображение заметки от координатора за выбранную дату
...
- Уведомление координатора (для Premium)
...
#### 6.4 Информация о координаторе (Premium)
- Имя координатора
- Email координатора
- Ссылка на чат с координатором
...
### 7. Чат с координатором (Premium)

**Описание:** Real-time чат между клиентом и координатором
...
```
Как Premium клиент
Я хочу общаться с координатором в реальном времени
...
## Функции для координаторов (Coordinator)

### 1. Кабинет координатора

**Описание:** Главная страница координатора со списком клиентов

**Маршрут:** `/app/coordinator`

**Функциональность:**

#### 1.1 Список клиентов
- Отображение всех Premium клиентов координатора
...
- Клик по клиенту → `/app/coordinator/[clientId]`
...
**Описание:** Просмотр данных клиента координатором

**Маршрут:** `/app/coordinator/[clientId]`
...
#### 2.3 Заметки координатора
- Date Picker для выбора даты
- Текстовое поле для ввода заметки
- Сохранение заметки в `coordinator_notes`
- Оптимистичные обновления UI
...
**Маршрут:** `/app/coordinator/invites`
...
- Изменение роли (client ↔ coordinator)
