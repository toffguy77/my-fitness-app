...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'reset_tokens') THEN
        EXECUTE 'GRANT ALL ON TABLE reset_tokens TO PUBLIC';
        RAISE NOTICE 'Granted permissions on reset_tokens table';
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'password_reset_attempts') THEN
