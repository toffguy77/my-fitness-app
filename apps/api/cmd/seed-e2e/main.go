...
// подключения.
//
// Цена ошибки несимметрична. Фикстуры создаются с паролем, посчитанным
// `bcrypt.MinCost` — стоимостью 4 вместо 10. Для тестов это правильно: иначе
// прогон тратит секунды на хэширование. В боевой базе такая же строка — это
// учётная запись с полными правами и намеренно ослабленным паролем, причём
// одна из фикстур имеет роль super_admin.
//
// На проде этого не случалось: проверено 2026-09-15, тестовых учёток там ноль.
// Защита стоит здесь не по следам происшествия, а потому что стоит она дёшево,
// а разбор последствий — нет.
//
