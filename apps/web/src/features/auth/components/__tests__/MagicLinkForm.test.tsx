...
describe('MagicLinkForm', () => {
    ...
    // подсказка войти по паролю, 400 — про согласия); любой другой код обязан
    ...
    // просто добавив условие на новый статус. Проверено не только на 404:
    // 409 (гонка создания аккаунта — тот же статус, что ConsumeMagicLink уже
    // отдаёт сегодня, см. handler.go) — самый правдоподобный кандидат на
    // будущую утечку, поэтому в списке, а не только гипотетический пример.
    // Сравнение с `messageFor` из того же модуля, что использует остальное
    // приложение, а не с захардкоженной строкой — так тест не подсказывает,
    // какой текст обойти.
    it.each([
        [404, 'not_found'],
        [409, 'conflict'],
        ...
        [422, 'validation'],
        [500, 'internal'],
    ])('код %i не заводит собственный текст — только общий словарь', async (status, code) => {
        const user = userEvent.setup()
        const error = new ApiError(status as number, { code })
