...
// Ловушка фокуса переехала в общий слой: её использует диалог подтверждения,
// а общему компоненту нельзя зависеть от модуля дашборда. Реэкспорт оставлен,
// чтобы существующие импорты из этого модуля продолжали работать.
export { useFocusTrap } from '@/shared/hooks/useFocusTrap'
